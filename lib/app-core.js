...
(function attachAppCore(root) {
  ...
  function deriveAuthOverlayState({ authed, role = null, routeRole = null, environment = '' } = {}) {
    const effectiveRole = role || (!authed && routeRole === 'admin' ? 'admin' : null);
    ...
    const principalLabel = authed ? formatPrincipalLabel(effectiveRole) : 'Not signed in';
    ...
    return {
      ...
      principalLabel,
      environmentLabel,
      showAdminControls: isAdmin && !!authed,
      authActionText: authed ? 'Logout' : 'Unlock',
      authActionLabel: authed ? 'Log out' : 'Unlock admin',
      logoutEnabled: true,
      logoutOpacity: '1'
    };
  }
  ...
  function deriveGlobalConnectionState({ authed, panes } = {}) {
    ...
    const unreadCount = list.reduce((sum, pane) => sum + Math.max(0, Number(pane?.unreadCount || 0)), 0);
    const attentionCount = list.filter((pane) => paneNeedsAttention(pane)).length;
    ...
    return { state, meta, connectedCount, disconnectedCount, unreadCount, attentionCount, total, ariaLabel };
  }
