...
function listItems(state, { queues, status } = {}) {
  ...
  return state.items
    .filter((it) => {
      if (it?.meta?.mergedInto) return false;
...
function statusRankForIssueDedupeMerge(status) {
  return TERMINAL_STATUSES.has(String(status || '').trim()) ? 0 : 1;
}
...
function compareIssueDedupeSurvivors(a, b) {
  const statusRank = statusRankForIssueDedupeMerge(b.status) - statusRankForIssueDedupeMerge(a.status);
...
function summarizeMergedIssueItem(item) {
  return {
    id: item.id,
    queue: item.queue,
    title: item.title,
...
function buildLegacyIssueDedupeGroups(state, { queues } = {}) {
  const queueSet = queues && queues.length ? new Set(queues) : null;
  const groups = new Map();

  for (const item of Array.isArray(state.items) ? state.items : []) {
    if (!item || item.meta?.mergedInto) continue;
    if (queueSet && !queueSet.has(item.queue)) continue;
    const key = canonicalizeIssueDedupeKey({
      dedupeKey: item.dedupeKey,
      title: item.title,
      instructions: item.instructions,
    ...
    const groupKey = `${item.queue || ''}\n${key}`;
    const group = groups.get(groupKey) || { queue: item.queue, canonicalKey: key, items: [] };
...
function planLegacyIssueDedupeMigration(state, { queues } = {}) {
  const normalizedQueues = Array.isArray(queues) ? queues.map((q) => String(q).trim()).filter(Boolean) : [];
  const groups = buildLegacyIssueDedupeGroups(state, { queues: normalizedQueues });

  return groups.map((group) => {
    const sorted = group.items.slice().sort(compareIssueDedupeSurvivors);
    ...
    return {
      ...
      mergedIds: merged.map((item) => item.id),
      count: sorted.length
    };
  });
}

function migrateLegacyIssueDupes(rootDir, { queues, dryRun = false, backup = true } = {}) {
  const { lockFile, stateFile, dir } = statePaths(rootDir);
  const normalizedQueues = Array.isArray(queues) ? queues.map((q) => String(q).trim()).filter(Boolean) : [];

  return withFileLock(lockFile, () => {
    const state = loadState(rootDir);
    const plan = planLegacyIssueDedupeMigration(state, { queues: normalizedQueues });
    const mergedCount = plan.reduce((sum, group) => sum + group.mergedIds.length, 0);
    if (dryRun || mergedCount === 0) {
      return { dryRun: Boolean(dryRun), groups: plan, mergedCount, backupPath: null };
    }

    let backupPath = null;
    if (backup) {
      ensureDir(dir);
      backupPath = path.join(dir, `work-queues.backup.${timestampForFile()}.json`);
    ...
    for (const group of plan) {
      ...
      survivor.dedupeKey = group.canonicalKey;
      survivor.result = survivor.result && typeof survivor.result === 'object' && !Array.isArray(survivor.result) ? survivor.result : {};
      survivor.result.migrationMerged = Array.isArray(survivor.result.migrationMerged) ? survivor.result.migrationMerged : [];

      for (const mergedId of group.mergedIds) {
        const merged = byId.get(mergedId);
        if (!merged) continue;
        survivor.result.migrationMerged.push(summarizeMergedIssueItem(merged));
        merged.status = 'done';
        merged.claimedBy = '';
        merged.claimedAt = '';
        merged.leaseUntil = 0;
        merged.dedupeKey = group.canonicalKey;
        merged.meta = cleanMeta({
          ...(merged.meta || {}),
          mergedInto: survivor.id,
          mergedAt,
          mergeRunId,
          mergeCanonicalKey: group.canonicalKey
        });
        merged.updatedAt = mergedAt;
      }
      survivor.meta = cleanMeta({ ...(survivor.meta || {}), mergeRunId, mergeCanonicalKey: group.canonicalKey });
      survivor.updatedAt = mergedAt;
    }

    saveState(rootDir, state);
    return { dryRun: false, groups: plan, mergedCount, backupPath };
  });
}
