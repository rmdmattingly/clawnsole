...
function itemTerminalAtMs(item) {
...
function archiveTerminalItems(rootDir, { queue = '', olderThanDays, previewOnly = false } = {}) {
  ...
  return withFileLock(lockFile, () => {
    ...
    for (const item of state.items) {
      const status = String(item?.status || 'ready').trim();
      const terminalAt = itemTerminalAtMs(item);
      const matchesQueue = !q || item?.queue === q;
      const eligible = matchesQueue && TERMINAL_STATUSES.has(status) && terminalAt !== null && terminalAt < cutoffMs;
