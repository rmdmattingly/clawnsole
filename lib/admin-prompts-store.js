    ...
    try {
      ...
      try {
        for (const p of prompts) {
          ...
          const now = Date.now()
          const promptId = String(p.id || randomId())
          insert.run(
            promptId,
          ...
          const history = Array.isArray(p.runHistory) ? p.runHistory : []
          history.forEach((run) => {
            if (!run || typeof run !== 'object') return
            const ts = Number(run.ts ?? run.at ?? run.lastRunAt ?? 0)
            if (!Number.isFinite(ts) || ts <= 0) return
            const idempotencyKey = String(run.idempotencyKey || `legacy:${promptId}:${ts}`)
            insertRun.run(
              String(run.id || randomId()),
              promptId,
              idempotencyKey,
              String(run.sessionKey || `agent:${String(p.agentId || 'main')}:admin:legacy`),
              String(run.status || run.lastStatus || 'unknown'),
              String(run.error || run.lastError || ''),
              ts,
              run.deliveredAt == null ? ts : Number(run.deliveredAt)
            )
          })
        }
        db.exec('COMMIT')
