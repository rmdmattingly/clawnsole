...
test('ctrl/cmd+shift+alt+g opens or focuses workqueue for active chat agent outside typing contexts', async ({ page }) => {
  test.setTimeout(180000);
  test.skip(!!app?.skipReason, app?.skipReason);
  ...
  const chatInput = page.locator('[data-pane][data-pane-kind="chat"] [data-pane-input]').first();
  const initialWorkqueueCount = await page.locator('[data-pane][data-pane-kind="workqueue"]').count();

  await chatInput.focus();
  await page.keyboard.press('ControlOrMeta+Shift+Alt+G');
  await expect(page.locator('[data-pane][data-pane-kind="workqueue"]')).toHaveCount(initialWorkqueueCount);
  await expect(chatInput).toBeFocused();
  await expect(page.getByTestId('shortcut-blocked-toast').last()).toContainText('Shortcut paused while typing');
