...
test('workqueue pane: filter summary chips show counts and remove filters', async ({ page }) => {
  ...
  await expect(wqPane.locator('[data-wq-statusline]')).toContainText('Showing 1 of 3 items');
  await expect(summary).toContainText('Item search alternate repo');

  await summary.locator('[data-wq-clear-all-filters]').click();
  await expect(wqPane.locator('[data-wq-statusline]')).toContainText('Showing 2 of 3 items');
  await expect(summary).not.toContainText('Item search alternate repo');
  await expect(wqPane.locator('[data-wq-queue-custom]')).toHaveValue(queue);
});

test('workqueue pane: queue filter and item search are separate controls', async ({ page }) => {
  test.setTimeout(180000);
  test.skip(!!env?.skipReason, env?.skipReason);
  ...
  const itemSearch = wqPane.locator('[data-wq-search]');
  ...
  await queueSearch.fill('alternate repo');
  await expect(wqPane.locator('.wq-row')).toHaveCount(2);

  await itemSearch.fill('alternate repo');
  await expect(wqPane.locator('.wq-row')).toHaveCount(1);
  await expect(wqPane.locator('[data-wq-filter-summary]')).toContainText('Item search alternate repo');

  await wqPane.locator('.wq-row').first().click();
  await page.keyboard.press('/');
  await expect(itemSearch).toBeFocused();

  await itemSearch.fill('missing-query');
  await expect(wqPane.locator('.wq-row')).toHaveCount(0);
