...
let recurringPromptState = { prompts: [] };
...
async function loadRecurringPrompts() {
  if (!globalElements.recurringPromptStatus) return;
  globalElements.recurringPromptStatus.textContent = '';
  try {
    const res = await fetch('/api/recurring-prompts', { credentials: 'include' });
    const data = await res.json().catch(() => null);
    if (!res.ok || !data?.ok) {
      globalElements.recurringPromptStatus.textContent = 'Failed to load (' + (data?.error || res.status) + ').';
      return;
    }
    recurringPromptState.prompts = Array.isArray(data.prompts) ? data.prompts : [];
    renderRecurringPromptList();
  } catch (err) {
    globalElements.recurringPromptStatus.textContent = 'Failed to load (' + String(err) + ')';
  }
}

async function loadRecurringPromptAgents() {
  const sel = globalElements.recurringPromptAgent;
  if (!sel) return;
  sel.innerHTML = '';
  try {
    ...
    agents.forEach((agent) => {
      const opt = document.createElement('option');
      opt.value = agent.id;
      opt.textContent = agent.displayName ? agent.displayName + ' (' + agent.id + ')' : agent.id;
      sel.appendChild(opt);
    });
...
function renderRecurringPromptList() {
  const root = globalElements.recurringPromptList;
  if (!root) return;
  const prompts = recurringPromptState.prompts || [];
  if (prompts.length === 0) {
    root.innerHTML = '<div class="hint">No recurring prompts yet.</div>';
    return;
  }

  root.innerHTML = prompts
    .slice()
    .sort((a, b) => (a.createdAt || 0) - (b.createdAt || 0))
    .map((p) => {
      const enabled = p.enabled !== false;
      const errorHtml = p.lastError
        ? '<div class="hint" style="margin-top: 6px; color: #ff9a9a;">' + escapeHtml(p.lastError) + '</div>'
        : '';
      return (
        '<div style="padding: 10px; border: 1px solid rgba(255,255,255,0.08); border-radius: 10px; margin-bottom: 10px;">' +
        '<div style="display:flex; justify-content: space-between; gap: 10px;">' +
        '<div>' +
        '<div style="font-weight: 600;">' + escapeHtml(p.title || 'Recurring prompt') + '</div>' +
        '<div class="hint">agent: <code>' + escapeHtml(p.agentId || '') + '</code> · every <code>' +
        escapeHtml(String(p.intervalMinutes || '')) + '</code> min · ' + (enabled ? 'enabled' : 'disabled') + '</div>' +
        '<div class="hint">last: ' + escapeHtml(formatTimestamp(p.lastRunAt)) + ' · next: ' +
        escapeHtml(formatTimestamp(p.nextRunAt)) + ' · status: ' + escapeHtml(String(p.lastStatus || '')) + '</div>' +
        '</div>' +
        '<div style="display:flex; gap: 8px; align-items: start;">' +
        '<button class="secondary" type="button" data-recurring-toggle="' + escapeHtml(p.id) + '">' +
        (enabled ? 'Disable' : 'Enable') + '</button>' +
        '<button class="danger" type="button" data-recurring-delete="' + escapeHtml(p.id) + '">Delete</button>' +
        '</div>' +
        '</div>' +
        '<pre style="white-space: pre-wrap; margin: 10px 0 0; opacity: 0.9;">' + escapeHtml(p.message || '') + '</pre>' +
        errorHtml +
        '</div>'
      );
    })
  ...
  root.querySelectorAll('[data-recurring-delete]').forEach((btn) => {
    btn.addEventListener('click', async () => {
      const id = btn.getAttribute('data-recurring-delete');
      if (!id) return;
      if (!confirm('Delete this recurring prompt?')) return;
      await deleteRecurringPrompt(id);
    });
  });

  root.querySelectorAll('[data-recurring-toggle]').forEach((btn) => {
    btn.addEventListener('click', async () => {
      const id = btn.getAttribute('data-recurring-toggle');
      const prompt = (recurringPromptState.prompts || []).find((p) => p && p.id === id);
      if (!id || !prompt) return;
      await updateRecurringPrompt(id, { enabled: !(prompt.enabled !== false) });
    });
  });
}

async function createRecurringPromptFromUi() {
  if (!globalElements.recurringPromptCreateBtn) return;
  globalElements.recurringPromptStatus.textContent = '';
  const agentId = (globalElements.recurringPromptAgent?.value || '').trim() || 'main';
  const intervalMinutes = Number(globalElements.recurringPromptInterval?.value || 60) || 60;
  const message = (globalElements.recurringPromptMessage?.value || '').trim();
  const enabled = Boolean(globalElements.recurringPromptEnabled?.checked);
  if (!message) {
    globalElements.recurringPromptStatus.textContent = 'Message required.';
    return;
  }

  try {
    const res = await fetch('/api/recurring-prompts', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
      body: JSON.stringify({ title: 'Recurring prompt', agentId, intervalMinutes, message, enabled })
    });
    const data = await res.json().catch(() => null);
    if (!res.ok || !data?.ok) {
      globalElements.recurringPromptStatus.textContent = 'Create failed (' + (data?.error || res.status) + ').';
      return;
    }
    if (globalElements.recurringPromptMessage) globalElements.recurringPromptMessage.value = '';
    globalElements.recurringPromptStatus.textContent = 'Created.';
    await loadRecurringPrompts();
  } catch (err) {
    globalElements.recurringPromptStatus.textContent = 'Create failed (' + String(err) + ')';
  }
}

async function updateRecurringPrompt(id, patch) {
  if (!id) return;
  try {
    const res = await fetch('/api/recurring-prompts/' + encodeURIComponent(id), {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
    ...
    const data = await res.json().catch(() => null);
    if (!res.ok || !data?.ok) {
      globalElements.recurringPromptStatus.textContent = 'Update failed (' + (data?.error || res.status) + ').';
      return;
    }
    await loadRecurringPrompts();
  } catch (err) {
    globalElements.recurringPromptStatus.textContent = 'Update failed (' + String(err) + ')';
  }
}
...
async function deleteRecurringPrompt(id) {
  ...
  try {
    ...
    const data = await res.json().catch(() => null);
    if (!res.ok || !data?.ok) {
      globalElements.recurringPromptStatus.textContent = 'Delete failed (' + (data?.error || res.status) + ').';
      return;
    }
    globalElements.recurringPromptStatus.textContent = 'Deleted.';
    await loadRecurringPrompts();
  } catch (err) {
    globalElements.recurringPromptStatus.textContent = 'Delete failed (' + String(err) + ')';
  }
}
